...
class HuyaSite implements LiveSite {
  ...
  Future<String> getPlayUrl(HuyaLineModel line, int bitRate) async {
    // 使用 Web 方式生成 anti-code，参考 chen-zeong/DTV 项目
    // 这种方式比 tars 协议更稳定，可以解决断流问题
    var antiParams = generateWebAntiCode(line.streamName, line.flvAntiCode);
    
    var url = '${line.line}/${line.streamName}.flv?$antiParams';
    
    // 对腾讯 CDN 进行特殊处理
    if (line.cdnType.toLowerCase() == 'tx') {
      url = url.replaceAll('&ctype=tars_mp', '&ctype=huya_webh5');
      url = url.replaceAll('&fs=bhct', '&fs=bgct');
    }
  ...
  /// 生成 Web anti-code 参数，参考 DTV 项目实现
  /// 核心算法：重新计算 wsSecret、wsTime、seqId 等参数
  String generateWebAntiCode(String streamName, String antiCode) {
    // 清理 anti-code
    var sanitized = antiCode.replaceAll('&amp;', '&');
    sanitized = sanitized.replaceAll(RegExp(r'^[?&]+'), '');
    
    var params = Uri.splitQueryString(sanitized);
    
    // 获取必要参数
    var fmValue = params['fm'] ?? '';
    var ctype = params['ctype'] ?? 'huya_webh5';
    var fs = params['fs'] ?? 'bgct';
    
    // 解码 fm 参数获取 wsSecret 前缀
    String wsPrefix;
    try {
      var fmDecoded = Uri.decodeComponent(fmValue);
      var fmBytes = base64.decode(fmDecoded);
      var fmPlain = utf8.decode(fmBytes);
      wsPrefix = fmPlain.split('_').first;
    } catch (e) {
      wsPrefix = '';
    }
    ...
    // 生成参数
    var paramsT = 100;
    var sdkVersion = 2403051612;
    var t13 = DateTime.now().millisecondsSinceEpoch;
    var sdkSid = t13;
    
    // 生成随机 uid
    var rng = Random();
    var uid = 1400000000000 + rng.nextInt(9999999);
    var seqId = uid + sdkSid;
    
    // 计算 wsTime（当前时间 + 110624ms 转换为16进制）
    var wsTime = ((t13 + 110624) ~/ 1000).toRadixString(16);
    
    // 生成 uuid
    var uuidSeed = (t13 % 10000000000) * 1000 + rng.nextInt(1000);
    var initUuid = uuidSeed % 4294967295;
    
    // 计算 wsSecret
    var wsSecretHash = md5.convert(utf8.encode('$seqId|$ctype|$paramsT')).toString();
    var wsSecretPlain = '${wsPrefix}_${uid}_${streamName}_${wsSecretHash}_$wsTime';
    var wsSecret = md5.convert(utf8.encode(wsSecretPlain)).toString();
    ...
    var queryParams = {
      ...
      'ver': '1',
      'fs': fs,
      'uuid': initUuid.toString(),
      'u': uid.toString(),
      't': paramsT.toString(),
      'sv': sdkVersion.toString(),
